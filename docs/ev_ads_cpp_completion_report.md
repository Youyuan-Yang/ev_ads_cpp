...
- 根据 RK3588 实机 dmesg 识别结果新增摄像头 udev 固定路径：前置 `ocal4(1bcf:28c5)` → `/dev/ev_ads/front_camera`，后置鱼眼 `A68-1600W(1bcf:2281)` → `/dev/ev_ads/rear_fisheye`，驾驶员人脸 `WebCamera(32e6:9221)` → `/dev/ev_ads/driver_face`。
