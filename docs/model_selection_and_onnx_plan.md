...
- `onnx_yolo_detector.hpp/cpp`：OpenCV DNN 加载 ONNX。
- `rear_blind_spot_node`：后置 ONNX 推理接口。
- `driver_attention_node`：YuNet + DMS YOLO ONNX 组合推理接口。
- `ev_ads_runtime.launch.xml`：模型路径启动参数。
- `models/README.md`：模型放置规范。
- `models/onnx/rear_yolo.onnx`：由官方 `yolo11n.pt` 导出并完成本机 OpenCV DNN 加载测试。
- `onnx_yolo_detector.cpp`：已兼容端到端输出格式。
- `rear_blind_spot_node`：已加入鱼眼去畸变入口。
